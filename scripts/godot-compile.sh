...
RUN_ID="$(qq_run_record_start "compile" "godot-compile" "godot-cli" "godot-headless" "Godot compile/check started" | $QQ_PY -c 'import json,sys; print(json.load(sys.stdin)["run_id"])')"
TMP_OUTPUT="$(mktemp)"
