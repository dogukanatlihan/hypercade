-- HYPERCADE D1 (Cloudflare SQLite) schema — mirror of server/db.ts.
-- D1 manages journaling/WAL itself, so the better-sqlite3-only pragmas
-- (journal_mode=WAL, synchronous, foreign_keys, mmap) are intentionally dropped.
-- Kept Postgres-portable: TEXT/INTEGER/REAL only, no sqlite-isms.
